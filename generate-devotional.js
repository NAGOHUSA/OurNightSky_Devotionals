...
jobs:
  generate-devotional:
    ...
    - name: Generate enhanced devotional
      ...
      run: |
        echo "🌟 Starting Enhanced Devotional Generation"
        echo "Force regenerate: $FORCE_REGENERATE"
        
        # Run the enhanced generator with error handling
        if node generate-devotional.js; then
          echo "✅ Devotional generation completed successfully"
        else
          echo "❌ Primary generation failed, creating fallback"
          node -e "
          const fs = require('fs');
          const today = new Date().toISOString().split('T')[0];
          
          // Create emergency fallback devotional
          const fallback = {
            id: \`devotional-\${today}\`,
            date: today,
          ...
          console.log('🆘 Emergency fallback devotional created');
          "
        fi
        
    - name: Validate generated content
      run: |
        echo "🔍 Validating generated devotional..."
        node -e "
        const fs = require('fs');
        const today = new Date().toISOString().split('T')[0];
        ...
        if (!fs.existsSync(filePath)) {
          console.error('❌ No devotional file generated!');
          process.exit(1);
        }
        ...
        try {
          ...
          console.log('✅ Devotional validation passed');
    ...
    - name: Commit and push devotional
      run: |
        ...
        # Add generated files
        git add devotionals/
        git add content_tracker.json || true  # Don't fail if file doesn't exist
        
        # Check if there are changes to commit
        if git diff --staged --quiet; then
          echo "📝 No changes to commit"
        else
          # Get today's devotional info for commit message
          TODAY=$(date +%Y-%m-%d)
          if [ -f "devotionals/${TODAY}.json" ]; then
            ...
            git commit -m "✨ Enhanced Daily Devotional: ${TITLE}

📅 Date: ${TODAY}
🌙 Generated by OurNightSky Enhanced System
🎯 Features: Unique content, mental health themes, astronomical accuracy"
          else
            git commit -m "📝 Daily devotional update for ${TODAY}"
          fi
    ...
    - name: Generation summary
      ...
      run: |
        ...
        if [ -f "devotionals/${TODAY}.json" ]; then
          echo "✅ Devotional file created successfully"
          ...
          echo "📚 Total devotionals: ${TOTAL}"
          
        else
          echo "❌ No devotional file found"
        fi
        
        echo "🎉 Enhanced devotional generation process complete!"
